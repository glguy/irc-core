...
data ActiveExtension = ActiveExtension
  ...
  , aeTimers  :: ![(UTCTime, TimerEntry)]
  , aeNextTimer :: !Int
  , aeThreads :: !Int
  , aeLive    :: !Bool
  }

data TimerEntry = TimerEntry !Int !(FunPtr TimerCallback) !(Ptr ())
...
popTimer ae =
  do let timers = aeTimers ae
     guard (aeLive ae)
     ((time, TimerEntry timerId fun ptr), timers') <- minView timers
     let ae' = ae { aeTimers = timers' }
     return (time, fromIntegral timerId, fun, ptr, ae')

minView [] = Nothing
minView xs = Just (x, filter (\(_,TimerEntry j _ _) -> i /= j) xs)
  where
    x@(_, TimerEntry i _ _) = minimumBy (comparing fst) xs
...
pushTimer time fun ptr ae = entry `seq` ae' `seq` (i, ae')
  where
    entry = TimerEntry i fun ptr
    i     = aeNextTimer ae
    ae'   = ae { aeTimers = (time, entry) : aeTimers ae
               , aeNextTimer = i + 1 }
...
cancelTimer timerId ae =
  do (_, TimerEntry _ _ ptr) <- find (\(_, TimerEntry i _ _) -> timerId == i) (aeTimers ae)
     return (ptr, ae { aeTimers = filter (\(_,TimerEntry i _ _) -> timerId /= i) (aeTimers ae)})
